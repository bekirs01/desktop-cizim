...
import { CanvasManager } from "./app/drawing/CanvasManager.js";
...
function pushPdfHistory() {
  ...
  if (pdfHistoryStack.length > 50) {
    pdfHistoryStack.shift();
    pdfHistoryIndex--;
  } else {
    pdfHistoryIndex = pdfHistoryStack.length - 1;
  }
}
...
function pushPptxHistory() {
  ...
  if (pptxHistoryStack.length > 50) {
    pptxHistoryStack.shift();
    pptxHistoryIndex--;
  } else {
    pptxHistoryIndex = pptxHistoryStack.length - 1;
  }
}
...
function drawShapeToCtx(ctx, sh, w, h, sx) {
  CanvasManager.drawShapeToCtx(ctx, sh, w, h, sx, drawColor, drawLineWidth);
}
